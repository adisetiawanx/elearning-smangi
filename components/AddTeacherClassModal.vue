        ...
        <div class="flex min-h-full items-center justify-center p-4">
          ...
          <HeadlessDialogPanel class="w-full max-w-xl rounded bg-white p-5">
            <HeadlessDialogTitle class="border-b pb-3 font-medium text-lg"
              >Tambahkan Guru ke Kelas</HeadlessDialogTitle
            >

            <!-- Form for adding Siswa -->
            <form @submit.prevent="addTeacherToClass" class="mt-3">
              <div class="mb-4">
                <label
                  for="kelas"
                ...
                <select
                  id="kelas"
                  v-model="choosedClass"
                  class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                >
                  <option value="">Pilih Kelas</option>
                  <option
                    v-for="(kelas, kelasIndex) in kelasList"
                    :key="kelasIndex"
                    :value="kelas.name"
                  >
                    {{ kelas.name }}
...
const { addTeacherToClass: postTeacherToClass } = useTeacher();
const { getListKelas } = useKelas();

const choosedClass = ref<any>("");
...
const addTeacherToClass = async () => {
  await postTeacherToClass(props.teacherId, choosedClass.value);
  await props.fetchTeacherData();
  closeModal();
};

onMounted(async () => {
  kelasList.value = await getListKelas({
    take: undefined,
    skip: 0,
  });
});
